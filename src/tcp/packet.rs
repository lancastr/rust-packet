...
impl<B: AsRef<[u8]>> Packet<B> {
    ...
    pub fn flags(&self) -> Flags {
        Flags::from_bits((&self.buffer.as_ref()[12..]).read_u16::<BigEndian>().unwrap() & 0b1_1111_1111).unwrap()
    }
...
impl<B: AsRef<[u8]> + AsMut<[u8]>> Packet<B> {
    ...
    pub fn set_flags(&mut self, value: Flags) -> Result<&mut Self> {
        ...
        Cursor::new(&mut self.header_mut()[12..]).write_u16::<BigEndian>((u16::from(old)) << 12 | value.bits())?;
...
mod test {
    ...
    fn values() {
        let raw = [
            0x45u8, 0x00, 0x00, 0x3c, 0xc8, 0xa5, 0x40, 0x00, 0x40, 0x06, 0x9f, 0xd5, 0xc0, 0xa8, 0x01, 0x89, 0x08, 0x08, 0x08, 0x08, 0x9b, 0x8a, 0x00, 0x50, 0xde, 0x67, 0xc7, 0x4a, 0x00, 0x00, 0x00,
            0x00, 0xa0, 0x02, 0x72, 0x10, 0x3f, 0x5f, 0x00, 0x00, 0x02, 0x04, 0x05, 0xb4, 0x04, 0x02, 0x08, 0x0a, 0x59, 0x2b, 0x29, 0x97, 0x00, 0x00, 0x00, 0x00, 0x01, 0x03, 0x03, 0x07,
        ];
