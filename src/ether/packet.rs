...
impl<B: AsRef<[u8]>> Packet<B> {
    ...
    pub fn destination(&self) -> MacAddress {
        MacAddress::from_bytes(&self.buffer.as_ref()[0..6]).unwrap()
    }

    /// MAC address for the source.
    pub fn source(&self) -> MacAddress {
        MacAddress::from_bytes(&self.buffer.as_ref()[6..12]).unwrap()
    }
...
impl<B: AsRef<[u8]> + AsMut<[u8]>> Packet<B> {
    ...
    pub fn set_destination(&mut self, value: MacAddress) -> Result<&mut Self> {
        self.buffer.as_mut()[0..6].copy_from_slice(value.as_bytes());

        Ok(self)
    }

    /// Source MAC address.
    pub fn set_source(&mut self, value: MacAddress) -> Result<&mut Self> {
        self.buffer.as_mut()[6..12].copy_from_slice(value.as_bytes());
...
mod test {
    ...
    fn values() {
        let raw = [
            0x00u8, 0x23, 0x69, 0x63, 0x59, 0xbe, 0xe4, 0xb3, 0x18, 0x26, 0x63, 0xa3, 0x08, 0x00, 0x45, 0x00, 0x00, 0x42, 0x47, 0x07, 0x40, 0x00, 0x40, 0x11, 0x6e, 0xcc, 0xc0, 0xa8, 0x01, 0x89, 0xc0,
            0xa8, 0x01, 0xfe, 0xba, 0x2f, 0x00, 0x35, 0x00, 0x2e, 0x1d, 0xf8, 0xbc, 0x81, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x03, 0x61, 0x70, 0x69, 0x0c, 0x73, 0x74, 0x65,
